...
class AllegroHandTrackingGeneralist(BaseTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render,
        ...
        # #5: task-space PALM position tracking (-coef*||sim_palm - ref_palm||) -> keep palm on the natural reference (anti-hug)
        self.palm_pos_rew = (os.environ.get('PALM_POS_REW', '0') == '1')
        self.palm_pos_coef = float(os.environ.get('PALM_POS_COEF', '2.0'))
        self.ref_palm_pos = None
        # #5: boost wrist trans/rot tracking coefs (pin the wrist to reference); default keeps original
        if 'GLB_TRANS_COEF' in os.environ:
        ...
        # SPIDER-style contact guidance: positive reward exp(-beta*||sim_tip - target_contact||) for
        # the fingers that SHOULD contact the object (from geometric contact detection), with the
        # contact point transformed by the LIVE object pose. Drives real grasp establishment.
        self.contact_guide = (os.environ.get('CONTACT_GUIDE', '0') == '1')
        self.contact_guide_coef = float(os.environ.get('CONTACT_COEF', '1.0'))
        self.contact_guide_beta = float(os.environ.get('CONTACT_BETA', '30.0'))
        # CG_HUBER_DELTA: 0 -> keep exp(-beta*d) reward (default). >0 -> Huber PENALTY on contact dist:
        #   near (d<=delta): 0.5*d^2 quadratic;  far (d>delta): delta*(d-0.5*delta) linear.
        #   gives gradient everywhere (exp had ~none beyond ~3cm) but far stays linear (no L2 blow-up).
        self.contact_guide_huber_delta = float(os.environ.get('CG_HUBER_DELTA', '0.0'))
        self.tot_contact_pos_local = None
        self.tot_contact_flag = None
        self.ref_contact_world = None
        self.ref_contact_flag = None
        print(f"[CONTACT_GUIDE] contact_guide={self.contact_guide} coef={self.contact_guide_coef} beta={self.contact_guide_beta} huber_delta={self.contact_guide_huber_delta}")
        ...
        self.action_rate_rew = (os.environ.get('ACTION_RATE', '0') == '1')
        self.action_rate_coef = float(os.environ.get('ACTION_RATE_COEF', '0.01'))
        self.prev_actions = None
        self.prev_prev_actions = None
        print(f"[ACTION_RATE] action_rate_rew={self.action_rate_rew} coef={self.action_rate_coef}")
        # SOFT joint-limit penalty (soft anti-reverse-joint, alternative to hard CLIP_DOF): quadratic
        # hinge on the ACHIEVED finger qpos beyond URDF [lower,upper], BOTH directions, all 20 finger
        # joints (skip the 6 global dofs). -coef * sum_j [relu(lo-q)^2 + relu(q-hi)^2]. Teaches the policy
        # to stay within limits (no chatter / no slowdown, unlike hard clip). Reference sits AT the limit
        # so it gets ~0 penalty. Default off; applied Python-side (after the fair call -> fair stays clean).
        self.soft_limit_rew = (os.environ.get('SOFT_LIMIT', '0') == '1')
        self.soft_limit_coef = float(os.environ.get('SOFT_LIMIT_COEF', '1.0'))
        print(f"[SOFT_LIMIT] soft_limit_rew={self.soft_limit_rew} coef={self.soft_limit_coef}")
    ...
    def _prepare_expert_traj_infos(self,):
        ...
        # ---- SPIDER-style contact guidance: load per-instance contact data (contact_pos_local (T,5,3) +
        # contact_flag (T,5)) generated by wuji_pipeline/generate_contact_guidance.py, stacked in the
        # same instance order as data_inst_tag_list. Disabled gracefully if any file is missing. ----
        if self.contact_guide:
            # CONTACT_SUBDIR selects which contact source: 'contact' = A版(wuji投影,默认),
            # 'contact_grab' = B版(GRAB真值). 切换实验只需 export CONTACT_SUBDIR=contact_grab。
            _contact_subdir = os.environ.get('CONTACT_SUBDIR', 'contact')
            _contact_dir = os.path.join(os.path.dirname(self.tracking_save_info_fn.rstrip('/')), _contact_subdir)
            print(f"[CONTACT_GUIDE] contact source dir = {_contact_dir}")
            _cp, _cf, _ok = [], [], True
            for _tag in self.data_inst_tag_list:
                _tag_s = _tag
                if not isinstance(_tag_s, str):
                    try:
                        _tag_s = _tag_s[0]
                    except Exception:
                        _tag_s = str(_tag)
                if not getattr(self, '_cg_tag_dbg', False):
                    print(f"[CONTACT_GUIDE][dbg] tag type={type(_tag).__name__} repr={repr(_tag)[:70]} -> tag_s={_tag_s}")
                    self._cg_tag_dbg = True
                _t = os.path.basename(str(_tag_s)).replace('_nf_300', '')
                _fn = os.path.join(_contact_dir, f"{_t}_contact.npy")
                if not os.path.exists(_fn):
                    print(f"[CONTACT_GUIDE] MISSING contact file {_fn} -> disabling contact guidance"); _ok = False; break
                _cd = np.load(_fn, allow_pickle=True).item()
                _cp.append(_cd['contact_pos_local'].astype(np.float32)); _cf.append(_cd['contact_flag'].astype(np.float32))
            if _ok and len(_cp) > 0:
                self.tot_contact_pos_local = torch.from_numpy(np.stack(_cp, axis=0)).float().to(self.rl_device)  # (n_insts, T, 5, 3)
                self.tot_contact_flag = torch.from_numpy(np.stack(_cf, axis=0)).float().to(self.rl_device)        # (n_insts, T, 5)
                print(f"[CONTACT_GUIDE] loaded contact data for {len(_cp)} insts: pos {tuple(self.tot_contact_pos_local.shape)} flag {tuple(self.tot_contact_flag.shape)}")
            else:
                self.contact_guide = False
    ...
    def compute_reward(self, actions, id=-1):
        ...
        # #4/#5 per-frame reference link positions (fingertips + palm) from FPOS data (link_key_to_link_pos)
        _have_links = (self.finger_pos_rew or self.palm_pos_rew) and getattr(self, 'tot_key_to_tot_link_pos', None) is not None and 'right_palm_link' in self.tot_key_to_tot_link_pos
        def _ref_link(_k):
            _lp = batched_index_select(self.tot_key_to_tot_link_pos[_k], self.env_inst_idxes, dim=0)               # nn_envs x nn_frames x 3
            return batched_index_select(_lp, cur_progress_buf.unsqueeze(-1), dim=1).squeeze(1)                      # nn_envs x 3
        if _have_links and self.finger_pos_rew and 'right_finger5_tip_link' in self.tot_key_to_tot_link_pos:
            self.ref_fingertip_pos = torch.stack([_ref_link(f'right_finger{_i}_tip_link') for _i in range(1, 6)], dim=1)  # nn_envs x 5 x 3
        else:
            self.ref_fingertip_pos = None
        self.ref_palm_pos = _ref_link('right_palm_link') if (_have_links and self.palm_pos_rew) else None           # nn_envs x 3
        if (self.finger_pos_rew or self.palm_pos_rew) and not getattr(self, '_fp_printed', False):
            print(f"[FINGER_POS] ref_fingertip_pos={None if self.ref_fingertip_pos is None else tuple(self.ref_fingertip_pos.shape)} ref_palm_pos={None if self.ref_palm_pos is None else tuple(self.ref_palm_pos.shape)} tot_link_keys={list(self.tot_key_to_tot_link_pos.keys()) if getattr(self,'tot_key_to_tot_link_pos',None) else None}")
            self._fp_printed = True

        # ---- contact guidance: per-frame contact targets transformed to WORLD by the live object pose ----
        if self.contact_guide and self.tot_contact_pos_local is not None:
            _ci = self.tot_contact_pos_local.shape[0]
            _cp = batched_index_select(self.tot_contact_pos_local.reshape(_ci, self.tot_contact_pos_local.shape[1], 15), self.env_inst_idxes, dim=0)  # N,T,15
            _cp = batched_index_select(_cp, cur_progress_buf.unsqueeze(-1), dim=1).squeeze(1).reshape(-1, 5, 3)   # N,5,3 (object-local)
            _cf = batched_index_select(self.tot_contact_flag, self.env_inst_idxes, dim=0)                          # N,T,5
            self.ref_contact_flag = batched_index_select(_cf, cur_progress_buf.unsqueeze(-1), dim=1).squeeze(1)    # N,5
            _q = self.object_rot.unsqueeze(1).expand(-1, 5, -1).reshape(-1, 4)
            self.ref_contact_world = (quat_apply(_q, _cp.reshape(-1, 3)).reshape(-1, 5, 3) + self.object_pos.unsqueeze(1))  # N,5,3 (world)
            if not getattr(self, '_cg_printed', False):
                print(f"[CONTACT_GUIDE] ref_contact_world={tuple(self.ref_contact_world.shape)} flag_mean={float(self.ref_contact_flag.mean()):.3f}")
                self._cg_printed = True
        else:
            self.ref_contact_world = None; self.ref_contact_flag = None
        ...
        # ---- SOFT joint-limit penalty (anti reverse-joint, all 20 finger joints, both directions,
        # quadratic hinge beyond URDF limits). Applied to the MAIN reward only (after fair -> fair clean).
        if self.soft_limit_rew:
            _q = self.shadow_hand_dof_pos[:, 6:]                       # finger joints (skip 6 global)
            _lo = self.shadow_hand_dof_lower_limits[6:].unsqueeze(0)   # (1, nfinger)
            _hi = self.shadow_hand_dof_upper_limits[6:].unsqueeze(0)
            _below = torch.clamp(_lo - _q, min=0.0)
            _above = torch.clamp(_q - _hi, min=0.0)
            _soft_pen = (_below * _below + _above * _above).sum(dim=-1)  # (num_envs,)
            self.rew_buf = self.rew_buf - self.soft_limit_coef * _soft_pen
            if int(self.progress_buf[0]) == 1:
                print(f"[SOFT_LIMIT] active: coef={self.soft_limit_coef} mean_pen={float(_soft_pen.mean()):.4f} max_pen={float(_soft_pen.max()):.4f} nfinger_dofs={_q.shape[1]}")
    ...
    def reset(self, env_ids=None, goal_env_ids=None): 
        ...
        if self.action_rate_rew and self.prev_actions is not None:
            self.prev_actions[env_ids] = 0
            self.prev_prev_actions[env_ids] = 0
    ...
    def pre_physics_step(self, actions):
        ...
        _new_actions = actions.clone().to(self.device)[..., : self.nn_hand_dof]
        if self.action_rate_rew:
            # shift action history (residual a_{t-2}, a_{t-1}); reset/realloc on shape change
            if self.prev_actions is None or self.prev_actions.shape != _new_actions.shape:
                self.prev_actions = torch.zeros_like(_new_actions)
                self.prev_prev_actions = torch.zeros_like(_new_actions)
            else:
                self.prev_prev_actions = self.prev_actions
                self.prev_actions = self.actions if (getattr(self, 'actions', None) is not None and self.actions.shape == _new_actions.shape) else torch.zeros_like(_new_actions)
        self.actions = _new_actions
...
def compute_hand_reward_tracking(
    ...
    # ---- SPIDER-style contact guidance: coef * mean_over_contact_fingers[ per-finger value ]
    # finger order = [th, ff, mf, rf, lf] = ref finger1..5; masked by per-finger contact flag.
    # per-finger value: huber_delta<=0 -> exp(-beta*d) bounded REWARD (default; ~no gradient beyond ~3cm);
    #                   huber_delta>0  -> -Huber_delta(d) PENALTY (normalized, far slope=1):
    #                   near(d<=delta) 0.5*d^2/delta quadratic, far(d>delta) (d-0.5*delta) linear
    #                   -> gradient everywhere (grad d/delta near, 1 far); far linear, no L2 blow-up. ----
    contact_guide_value = torch.zeros_like(reward)
    if contact_guide:
        if ref_contact_world is not None:
            if ref_contact_flag is not None:
                _hw = 1.0 if contact_guide_huber_delta > 0.0 else 0.0
                _dl = contact_guide_huber_delta
                _d = torch.norm(right_hand_th_pos - ref_contact_world[:, 0], p=2, dim=-1)
                _v = (1.0 - _hw) * torch.exp(-contact_guide_beta * _d) - _hw * torch.where(_d <= _dl, 0.5 * _d * _d / (_dl + 1e-9), _d - 0.5 * _dl)
                _cg = ref_contact_flag[:, 0] * _v
                _fs = ref_contact_flag[:, 0]
                _d = torch.norm(right_hand_ff_pos - ref_contact_world[:, 1], p=2, dim=-1)
                _v = (1.0 - _hw) * torch.exp(-contact_guide_beta * _d) - _hw * torch.where(_d <= _dl, 0.5 * _d * _d / (_dl + 1e-9), _d - 0.5 * _dl)
                _cg = _cg + ref_contact_flag[:, 1] * _v
                _fs = _fs + ref_contact_flag[:, 1]
                _d = torch.norm(right_hand_mf_pos - ref_contact_world[:, 2], p=2, dim=-1)
                _v = (1.0 - _hw) * torch.exp(-contact_guide_beta * _d) - _hw * torch.where(_d <= _dl, 0.5 * _d * _d / (_dl + 1e-9), _d - 0.5 * _dl)
                _cg = _cg + ref_contact_flag[:, 2] * _v
                _fs = _fs + ref_contact_flag[:, 2]
                _d = torch.norm(right_hand_rf_pos - ref_contact_world[:, 3], p=2, dim=-1)
                _v = (1.0 - _hw) * torch.exp(-contact_guide_beta * _d) - _hw * torch.where(_d <= _dl, 0.5 * _d * _d / (_dl + 1e-9), _d - 0.5 * _dl)
                _cg = _cg + ref_contact_flag[:, 3] * _v
                _fs = _fs + ref_contact_flag[:, 3]
                if right_hand_lf_pos is not None:
                    _d = torch.norm(right_hand_lf_pos - ref_contact_world[:, 4], p=2, dim=-1)
                    _v = (1.0 - _hw) * torch.exp(-contact_guide_beta * _d) - _hw * torch.where(_d <= _dl, 0.5 * _d * _d / (_dl + 1e-9), _d - 0.5 * _dl)
                    _cg = _cg + ref_contact_flag[:, 4] * _v
                    _fs = _fs + ref_contact_flag[:, 4]
                contact_guide_value = _cg / (_fs + 1e-6)
                reward = reward + contact_guide_coef * contact_guide_value

    # ---- idle-finger hold: 对不接触的指(contact_flag~0)拉向参考指尖位 -coef*||tip-ref_tip||.
    # 与 contact_guide 互补: (1-flag) 自动屏蔽接触指(它们由 contact_guide 管). 防止 cube 抓取里
    ...
    if action_rate_rew:
        if prev_actions is not None:
            if prev_prev_actions is not None:
                _fo = torch.sum((actions[..., :prev_actions.shape[-1]] - prev_actions) ** 2, dim=-1)
                _so = torch.sum((actions[..., :prev_actions.shape[-1]] - 2.0 * prev_actions + prev_prev_actions) ** 2, dim=-1)
                action_rate_value = _fo + _so
                reward = reward + (-action_rate_coef) * action_rate_value
